...
class DCBot {
  ...
  async set_default_server(id){
    let updated = await sql`update servers set def = true where id = ${id} returning id, def`
    return updated
  }
  async get_default_server(guild_id){
    let def_srv = await this.get_guild_servers(guild_id)
    return def_srv[0]
  }
  async get_guild_servers(guild_id){
    let servers = await sql`select * from servers where guild_id = ${guild_id} order by def desc`
    return servers
  }

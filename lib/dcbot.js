...
class DCBot {
  ...
  async save_server_perms(srv_obj){
    let save_obj = {
      operators: JSON.stringify(srv_obj.operators),
      delegates: JSON.stringify(srv_obj.delegates),
      channels: JSON.stringify(srv_obj.channels),
    }
    try {
      await sql`
        update mc_servers set ${sql(save_obj
          , 'operators'
          , 'delegates'
          , 'channels'
        )}
        where id = ${srv_obj.id}
      `
      let server = await this.get_servers(srv_obj.id)
      return server
    } catch (error) {
      console.error('ending here...')
      console.error(error)
      return false
    }
  }
  ...
  is_admin(msg){ // admins can add/edit/remove mc servers for the guild
    ...
    let guild_admins = bot.guilds[msg.channel.guild.id].admins
    let find_admin = guild_admins.find(a => a.id == msg.author.id)
    return !!find_admin
  }
  is_op(msg, srv){ // op has FULL console access to mc server
    if (this.is_owner(msg)) return true
    let srv_ops = srv.operators
    let op_user = srv_ops.find(o => o.type == 'users' && o.id == msg.author.id)

    let author_role_ids = msg.guild.roles.cache.filter(r => r.members.get(msg.author.id)).map(r => r.id)
    let op_role = srv_ops.find(d => d.type == 'roles' && author_role_ids.indexOf(d.id) !== -1)
    let some_op = op_user || op_role
    return !!some_op
  }
  is_delegate(msg, srv){ // delegates have limited console access via specific bot commands (eg: whitelisting)
    if (this.is_owner(msg)) return true
    let srv_delegates = srv.delegates
    let delegate_user = srv_delegates.find(d => d.type == 'users' && d.id == msg.author.id)

    let author_role_ids = msg.guild.roles.cache.filter(r => r.members.get(msg.author.id)).map(r => r.id)
    let delegate_role = srv_delegates.find(d => d.type == 'roles' && author_role_ids.indexOf(d.id) !== -1)
    let some_delegate = delegate_user || delegate_role
    return !!some_delegate
  }
